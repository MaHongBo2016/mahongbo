            ...
            <dl class="c_section c_section_service">
                ...
                <dd>
                	<div class="os_step_1"></div>
						...
						<form class="corp_form" id="bindForm1" method="post" action="<?php echo U('CompanyRegist/step1Handle');?>" >
							...
							<input type="text" value="<?php echo ($data["tel"]); ?>" placeholder="请输入你的手机号码或座机号码" maxlength="49" name="tel" id="tel">
							<span id="mytel" style="display:block;" class="error"> </span>
							<h3>
								<em class="redstar">*</em>接收简历邮箱 <span>（该邮箱为公司邮箱，审核通过后不可更改）</span>
							</h3>
							<input type="text" value="<?php echo ($data["email"]); ?>" placeholder="请输入你的公司邮箱作为接收简历邮箱" name="email" id="email">
							<span id="myemail" style="display:none;" class="error"></span>
...
<script type="" src="/Public/Home/js/jquery.js"></script>
<script>
	//验证手机号
	$(function(){
		$('#tel').blur(function(){
			...
			var tel = $(this).val();
			if(tel.length !=11 ){
				var str = '<span id="mytel" style="display:block; color: red" > 请填写正确的手机号</span>';
			$('#mytel').html(str);
			}
			//正则验证手机号
			...
			{
				var str = '<span id="mytel" style="display:block; color: red" > 请输入有效的手机号码！</span>';
				$('#mytel').html(str);
			}


		}).focus(function(){
			var str = '<span id="mytel" style="display:none; color: red" > </span>';
			$('#mytel').html(str);
		})
	})
...
<script>
	...
	$(function(){
		$("#email").blur(function(){
			...
			{
				alert("请输入正确的邮箱格式");
//				var str = '<span id="myemail" style="display:block; color: red" > 请输入正确的邮箱格式</span>';
//				$("#my1").html(str);
			}
		}).focus(function(){
			//var str = '<span id="myemail"  style="display:block; color: red;"></span>';
//			$('#myemail').hidden();
